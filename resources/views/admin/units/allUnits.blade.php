...
@section('content')
    ...
    <div class="container mt-3">
        <h1 style="text-align: center; color: #8e3151; font-family: MainFontBold">كل الوحدات</h1>
        <br>
                    ...
                    @foreach ($units as $unit)
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span>لا يوجد مالك</span>
                                @endif
                            </td>
                            <td>{{ $unit->owner->phone ?? 'غير متوفر' }}</td>
                            <td>
                            ...
                            <td>
                                @if (($unit->admin && $unit->admin->id == auth()->id()) || auth()->user()->isMaster)
                                    ...
                                    <a href="{{ route('admin.updateUnit', $unit->id) }}" class="btn btn-success btn-sm"
                                        style="background-color:#88304e">تعديل</a>
                                    <a href="{{ route('admin.deleteUnit', $unit->id) }}"
                                        class="btn btn-danger btn-sm">حذف</a>
                                @endif
                                @if (!$unit->managed_by && !auth()->user()->isMaster)
                                    <a href="{{ route('addmin.assing.unit', ['unitId' => $unit->id, 'adminId' => auth()->id()]) }}"
                                        class="btn btn-success btn-sm">استلام</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
