...
class UsersNotificationsController extends Controller
{
    ...
    public function index(){
        try{
            $notifications = UserOwnerNotification::where('owner_id', Auth::user()->id)
            ->where('sender_type', 'user')->with('user')->get();
    ...
    public function markAllAsRead(){
        try{
            UserOwnerNotification::where('owner_id', Auth::user()->id)
            ->where('sender_type', 'user')->update(['is_read' => true]);
            return response()->json([
                'status' => 'success',
