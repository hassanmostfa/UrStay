...
{
    public function bookUnit(Request $request, $owner, $unitId)
    {
        $user = Owner::find($owner);
        ...
        if ($user->is_completed === 1) {
            try {
                ...
                return redirect()->route('supervisor-dashboard')->with('success', 'تم ارسال طلب الحجز بنجاح!');
            } catch (\Throwable $th) {
    ...
    public function calculatePrice(Request $request)
{
    ...
    public function showBookingOpreationsPage($owner)
    {
        // Get All Accepted Bookings
        $bookings = Booking::where('booking_status' , 'accepted')
        ->where('owner_id' , $owner)
        ->get();
        return view('supervisor.bookings.allBookings' , compact('bookings'));
    }

    // Show New Bookings Requests
    public function showNewBookingRequests($owner)
    {
        // Get All Pending Bookings
        $bookings = Booking::where('booking_status' , 'in_negotiation')
        ->where('owner_id' , $owner)
        ->get();
        return view('supervisor.bookings.newBookingRequests' , compact('bookings'));
    }


    // Show Booking Details
    public function showBookingRequest($owner, $id)
    {
    ...
    public function acceptBookingRequest($owner, $id)
    {
        $booking = Booking::findOrFail($id);
        ...
        return redirect()->route('manage.owner.bookings', ['owner'=> $owner])->with('success', 'تم قبول الطلب بنجاح!');
    }
